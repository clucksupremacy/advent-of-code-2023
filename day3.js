...
function check_fraction(input) {
    ...
    // let ababa = dict.filter((obj) =>(obj.schematic_numbers.filter((number) => number.number & 1 != 0)));
    //console.log(ababa);

    // let result = dict.filter((obj) => obj.schematic_numbers.filter((sub_obj) => sub_obj.number < 20) % 1 !== 0);

    let result = dict.map((obj) => ({
        ...obj,
        schematic_numbers: obj.schematic_numbers.filter((sub_obj) => sub_obj.number % 1 !== 0),
      }));

    // let result = dict.map((element) => {
    //     return {...element, schematic_numbers: element.schematic_numbers.filter((schematic_numbers) => schematic_numbers.number % 1 !== 0)}
    // })
